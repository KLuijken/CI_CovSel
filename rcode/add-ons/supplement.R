...
provide_expressions <- function(sigmaA, sigmaL, sigmaY, alpha, beta, gamma, n){
  ...
  var_red   <- (1/(n-1)) * 
    ((sigmaY^2+ sigmaL^2*gamma^2)/(sigmaA^2+sigmaL^2*alpha^2)  - ((sigmaL^4*alpha^2*gamma^2)/(sigmaA^2+sigmaL^2*alpha^2)^2))
  mse_red   <- (n*sigmaL^4*alpha^2*gamma^2 - sigmaL^4*alpha^2*gamma^2 + 
                         sigmaA^2*sigmaY^2 + sigmaL^2*sigmaY^2*alpha^2 + 
                         sigmaA^2*sigmaL^2*gamma^2)/((n - 1)*(sigmaA^2 + sigmaL^2*alpha^2)^2)
...
generate_plot_input <- function(sigmaA, sigmaL, sigmaY, alpha, beta, gamma, n,
  ...
  plot_bias <- ggplot(bias) +
    ...
    scale_color_manual(name = "Model", values = c("Full" = "navyblue","Reduced"= "darkred")) +
  ...
  plot_var <- ggplot(var) +
    ...
    geom_point(aes(x = varied_parameter, y = reduced, color = "reduced")) + 
    geom_line(aes(x = varied_parameter, y = reduced, color = "reduced")) +
    scale_color_manual(name = "Model", values = c("full" = "navyblue","reduced"= "darkred")) + ylab(expression("Var("~hat(beta)~")")) + 
    xlab(element_text(varied_par_name))+
    ylim(0,0.025)+
    theme_classic()
...
# Expressions ----
#------------------------------------------------------------------------------#
# Set some randomly chosen values to store the objects
sigmaA <- 1.2
sigmaL <- 1.2
sigmaY <- 1.2
alpha  <- 0.3
...
# Helper expression
rLA    <- (sigmaL*alpha)/(sqrt(sigmaA^2+sigmaL^2*alpha^2))
...
var_red          <- (1/(n-1)) * 
  ((sigmaY^2+ sigmaL^2*gamma^2)/(sigmaA^2+sigmaL^2*alpha^2)  - ((sigmaL^4*alpha^2*gamma^2)/(sigmaA^2+sigmaL^2*alpha^2)^2))
mse_red_simple   <- bias_red^2 + var_red
mse_red_expr     <- (n*sigmaL^4*alpha^2*gamma^2 - sigmaL^4*alpha^2*gamma^2 + 
                       sigmaA^2*sigmaY^2 + sigmaL^2*sigmaY^2*alpha^2 + 
                       sigmaA^2*sigmaL^2*gamma^2)/((n - 1)*(sigmaA^2 + sigmaL^2*alpha^2)^2)
...
c(bias_full, m_bias_full)
c(var_full, m_var_full)
...
c(m_mse_full, mse_full_expr)
c(m_mse_red, mse_red_expr)
...
plot_samplesize <- generate_plot_input(sigmaA = 1,
                                       sigmaL = 1,
                                       sigmaY = 1,
                                       alpha = 1,
                                       beta = 1,
...
plot_instrument <- generate_plot_input(sigmaA = 1,
                                       sigmaL = 1,
                                       sigmaY = 1,
                                       alpha = 1,
                                       beta = 1,
...
plot_predictor <- generate_plot_input(sigmaA = 1,
                                       sigmaL = 1,
                                       sigmaY = 1,
...
plot_explained <- generate_plot_input(sigmaA = 1,
                                      sigmaL = 1,
