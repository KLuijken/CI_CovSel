...
Question: I could also have obtained the MRR as pY1 <- mean(data$Y[data$A==1]) ;  pY0 <- mean(data$Y[data$A==0]);  MRR <- pY1/pY0. What do you think is a better approach?
...
unadjusted <- logistf(Y~A,
  ...
  unadjusted_int  <- glm(data$Y ~ 
                           offset(as.matrix(data$Y, data$A) %*%
                                    coef(unadjusted)[-1]),
                         family = binomial)
